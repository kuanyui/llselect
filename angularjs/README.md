...
- `renderTriggerContent` - same idea for the trigger: the scopes behind the previous match / tag content die exactly when that content is rebuilt. Hooked at the content method, not `renderTrigger`, because `setItems` can rebuild the trigger content without running `renderTrigger` (it runs the whole trigger only when a chosen entry was dropped).
