...
- **Row scopes are released where their DOM dies**: wholesale in `renderPopupList` (open / filter / `setItems` / `rerender` rebuild the whole list), and per replaced row after `replacePopupListItemElInDom` (a multi toggle repaints one row; the old row's scope is released once its element is out of the DOM). Without the first, every filter keystroke leaks a scope per row; without the second, every toggle with `remove-selected="false"` does.
