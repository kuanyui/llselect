...
- **Trigger scopes are released where their DOM dies**: in the `renderTriggerContent` override. Every rebuild of the match / tag content passes through it, including `setItems`, which rebuilds the content without running `renderTrigger` (hooking `renderTrigger` leaked one chip scope per reload).
