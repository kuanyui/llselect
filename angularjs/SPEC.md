...
- **`$render` runs after every `setItems`**, in all three directives, so an async preset resolves once its item arrives. In `<llselect-single>` / `<llselect-multiple>`, `fromModelValue` resolves the value by MEMBERSHIP in the list (`select as` key, `track by` key, or identity), so a value the list lacks resolves to `undefined` - re-rendering never re-adds a dropped item. In `<ui-llselect>` there is no membership test without an alias: the selection is the model itself, ui-select's own semantics. Item comparison without `track by` is `angular.equals`, in both modes: multiple matches ui-select's `_isItemSelected` (`uiSelectController.js:332`) - a reload's structurally equal fresh object stays the one chosen item instead of becoming a second, addable one - and single needs it so `aria-selected` / open-focus survive an object-collection reload. `angular.equals` has no cycle guard, so items that reference each other require `track by`; in multiple mode ui-select's own comparison shares that hazard, while in single mode the deep compare is this bridge's addition.
...
- **`<ui-llselect>` rows must be interpolated before llselect measures them.** `$compile` alone does not fill bindings; AngularJS does that on the next digest, and llselect renders from native events, i.e. outside one. `compileTemplateSlot()` digests the row scope before returning the element (guarded on `$rootScope.$$phase`, since `$digest` throws inside a digest and the ambient one already traverses the new child). Without it the popup is positioned against the literal `{{p.name}}` text and resizes a tick later.
- **Trigger scopes are released where their DOM dies**: in the `renderTriggerContent` override. Every rebuild of the match / tag content passes through it, including `setItems`, which can rebuild the content without running `renderTrigger` (hooking `renderTrigger` leaked one chip scope per reload).
- **Releasing a template scope means `$destroy()` AND `angular.element(el).remove()`**: the core removes template DOM natively (`replaceChildren` / `replaceWith`), which never reaches jqLite's `cleanData`, so the global element-data cache entry - it holds the scope - would outlive the element (MEDIUM-94). Both leak tests pin the cache size.
- **Row scopes are released where their DOM dies**: wholesale in `renderPopupList` (open / filter / `setItems` / `rerender` rebuild the whole list), and per replaced row after `replacePopupListItemElInDom` (a multi toggle repaints one row; the old row's scope is released once its element is out of the DOM), plus the same element-connectivity release after `close()`, which detaches every row. Without the first, every filter keystroke leaks a scope per row; without the second, every toggle with `remove-selected="false"` does.
...
llselect resolves its settings bag once at construction, so `ll-placeholder` / `ll-filterable` / `ll-filter-fn` / `ll-popup-width-policy` / `ll-arrow` / `ll-checkboxes` / `ll-label-el` / `ll-popup-header-content-fn` / `ll-popup-footer-content-fn` / `ll-popup-list-leading-action-rows` / `ll-popup-list-trailing-action-rows` / `ll-popup-list-leading-rows-pinned` / `ll-popup-list-trailing-rows-pinned` are read once at link time and a later scope change does not move them. Only `ll-disabled` gets a `$watch`, because it maps onto the `setDisabled()` method rather than a setting. The third class is the event expression - `ll-on-open` / `ll-on-close` / `ll-on-filter-query-change` (which exposes `$query`) - evaluated on each event inside a digest, like `ng-click`, with expression errors routed to `$exceptionHandler`; the close that `destroy()` runs at scope teardown fires it too, as in core. Any new attribute has to be classified into one of these three before it is added. The two popup-slot attributes are read-once expressions with one twist: the core calls the function they evaluate to exactly once, in its constructor - so at link time, inside the link digest, unlike the row / trigger content fns the core calls per render outside any digest - and keeps the returned node for the instance's life. The two action-row attributes are read-once expressions too, evaluating to arrays of descriptors that the bridge clones; each descriptor's `onActivate` is then wrapped exactly like an event expression (it runs in a digest, errors to `$exceptionHandler`) and receives the widget instance as its first argument, so `sel.setChosenItems()` inside it takes the core-originated `$setViewValue` path (a user change: `ng-change` runs, the control turns dirty) while a plain model write stays programmatic; its `textFn` / `disabledFn` / `createContentElFn` run outside any digest, per render, like the row content fn, and receive nothing (MEDIUM-110).
