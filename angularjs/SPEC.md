...
| `ngDisabled`'s `priority: 100` orders link fns on the element; it does NOT guard the expression. Any directive can read the expression text via `attrs.ngDisabled` and `$watch` it itself (the ui-select bridge does exactly that). What priority makes impractical is SUPPRESSING ngDisabled's own attr toggle - out-prioritizing it needs `terminal`, which also kills `ngModel` on the element. | `priority: 100` - `angular.js:24549`; bridge read - `llselect-ui-select.js` `attrs.ngDisabled` |
...
| It has no item-to-string concept at all: its label is DOM and its filtering is an Angular filter expression. This is why `ll-label` exists. | `uisRepeatParser.parse` returns `itemName / keyName / source / filters / trackByExp / modelMapper` - no label - `uisRepeatParserService.js:61-75` |
...
- **A `select as` model value must be re-resolved after `setItems`**, because it is a key pointing into the list. Without the projection the model holds the item itself and llselect's own `setItems` already reconciled it - calling `$render` there would re-add an item that is no longer in the list.
- **When the chosen item vanishes from the list, the model keeps its value and the view goes empty.** This matches `ngOptions`, which shows its unknown option in the same situation. The shared consequence: `required` still passes, because the model is not empty.
...
llselect resolves its settings bag once at construction, so `ll-placeholder` / `ll-filterable` / `ll-popup-width-policy` / `ll-arrow` / `ll-checkboxes` are read once at link time and a later scope change does not move them. Only `ll-disabled` gets a `$watch`, because it maps onto the `setDisabled()` method rather than a setting. Any new attribute has to be classified this way before it is added.
