...
- **Without `track by`, items are compared with `angular.equals`.** One compare rule; its consequences:
  ...
  - Structurally equal duplicates are not supported; each mode has its own symptom:
    ...
    - Single: when the chosen item compares equal to duplicate rows, every one of those rows is marked selected. If nothing is chosen, or something else is, no duplicate is marked.
    - The project-wide duplicates policy is still open: MEDIUM-77 in the repo's `docs/llm/FIXME.md` (not shipped in the npm package).
