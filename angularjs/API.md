...
### `ll-popup-list-leading-action-rows`

**Expression** -> `popupListLeadingActionRows`. Commands rendered as option rows at the top of the list, inside the arrow-key ring, above the items.
...
### `ll-popup-list-trailing-action-rows`

**Expression** -> `popupListTrailingActionRows`. Same as [`ll-popup-list-leading-action-rows`](#ll-popup-list-leading-action-rows), at the bottom of the list, after the last item.
...
```html
<llselect-multiple ng-model="vm.langs" ll-popup-list-trailing-action-rows="vm.langRows"
