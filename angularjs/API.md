...
- **Without `track by`, items are compared with `angular.equals`.** One compare rule; its consequences:
  ...
  - Structurally equal duplicates are not supported; each mode has its own symptom:
    ...
    - Single: on a fresh list build, every row equal to the chosen item is marked selected. If nothing is chosen, or something else is, no duplicate is marked. (An in-popup click refreshes only the first equal row; see MEDIUM-77 below.)
