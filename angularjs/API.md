...
- **`allow-clear` is read once, as a static value.** Real ui-select `$observe`s the attribute, so an interpolated `allow-clear="{{vm.canClear}}"` can flip at runtime. In llselect `clearable` is a construction-time setting (the settings-freeze rule), so the bridge reads the attribute once at link time and an interpolation placeholder is not yet filled there - write a literal `allow-clear="true"` (or the bare attribute). To flip clearability at runtime, re-link the element (e.g. `ng-if`), like any other frozen setting.
