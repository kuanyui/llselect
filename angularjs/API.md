...
- **Without `track by`, items are compared with `angular.equals`.** One compare rule; its consequences:
  ...
  - Items that reference each other (parent / child back-references) REQUIRE `track by`: `angular.equals` recurses with no cycle guard, in ui-select and here alike.
  - Structurally equal duplicates collapse: a multiple model holding them renders one chip and is deduplicated on the first user change; a single list holding duplicate rows marks each of them selected. ui-select renders duplicates as-is; the UI here can never create them. The project-wide duplicates policy is still open (`docs/llm/FIXME.md` MEDIUM-77).
