...
test('an app policy directive reaches the instance via require and stays safe on native controls', () => {
  ...
  const a = boot({
    ...
    html: `<div ng-controller="C as vm">
      ...
      <ui-llselect own-disabled ng-model="vm.u" aria-label="U">
        <ui-llselect-match>{{$select.selected}}</ui-llselect-match>
        <ui-llselect-choices repeat="f in vm.fruits"></ui-llselect-choices>
      </ui-llselect>
      <input own-disabled type="text">
    </div>`,
    controller: function () { this.fruits = FRUITS.slice(); this.fruit = null; this.t = []; this.u = undefined; this.canEdit = true },
      ...
      $compileProvider.directive('ownDisabled', [function () {
        return {
          ...
          link: function (scope, element, attrs, ctrls) {
            const api = ctrls.single || ctrls.multiple || ctrls.ui
  ...
  assert.equal(triggers[2].getAttribute('data-disabled'), 'true', 'ui-llselect not disabled through instance()')
