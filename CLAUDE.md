...
- Docstrings and human-facing docs (TSDoc in src/, README, angularjs/API.md) are written for fast scanning - a reader with mild dyslexia must be able to follow them. Precise, short sentences; complete but never long-winded. Short never means telegraphic: when a fact is conditional, spell the connective out (if / when / while / otherwise) - write "If no query is active, every item is visible", never a fronted fragment like "No query active: every item is visible". Colon-fronted labels are for defining a term or introducing a list, not for carrying an if. Lead with ONE short sentence saying what the thing is, then put every further fact in its own bullet, one fact per bullet. Never glue several clauses into one sentence with dashes and nested parentheses. When one word bundles several conditions (e.g. which items count as "visible" for an operation), define it with a nested list: an intro bullet like "acts on items that satisfy all of the following:", then one condition per sub-bullet. For an overridable method (`protected`), the FIRST bullets say who calls it, where each parameter comes from, what each parameter means and - when a confusion is likely - what it is NOT, and the usual override shape (call `super`, edit the returned element); a reader must never have to guess whether they fill a parameter or the library does (the `createItemEl` `index` incident). Implementation and internals notes (what a method composes, complexity, subclass wiring) go LAST, at the tail of the docstring, after every reader-facing fact. TSDoc renders verbatim into the generated API reference, so docstrings are user-facing documentation held to this rule, not internal comments. Measurable limits for reader-facing explanations (README prose, Q&A / FAQ answers, demo hints), ruled after a rejected README section: the reason is the FIRST sentence; an answer is at most 3 bullets and 45 words in total; every sentence stays under 20 words; no parentheses in prose - an aside becomes its own sentence or is cut; a "why" answer carries no how-to, the recipe lives in the section that teaches the API; a question only a code reviewer would ask goes to DESIGN.md, not the README; at most two code names per line, since every backticked identifier is a decoding stop. Count before shipping. (Agent-facing docs under docs/llm/ keep their dense house style.)
- Plain language everywhere an explanation lives: docs, code comments, demo copy, and replies to the user. Never invent a term or lean on a vague metaphor when the concrete thing has a name - say "the option element the library builds", not "shell". A niche term is acceptable only if the same sentence defines it. When an existing doc uses an invented term, replace it with the concrete wording. The test for any term (user ruling): it must be either standard industry vocabulary a typical web developer knows without looking it up, or a term this project's own docs already define (README, DESIGN.md, naming-conventions.md). Everything else gets the concrete wording, in docs and in replies alike - "seam" (Feathers' legacy-code jargon for an overridable method) failed that test and was purged; say "overridable method" / "customization point". Replies to the user are held to the same fast-scanning standard as the docstring rule above (short sentences, one fact per bullet, a reader with mild dyslexia can follow them), and every reply must stand on its own: a reader with none of the earlier conversation must still follow it - name the thing, never "the issue above" or "as discussed". Context comes FIRST, before any tally, verdict or option label (user ruling, after a reply said "pin the whole block" without ever saying what the block was): define every noun the reply leans on in one plain sentence each - what the thing is, where it sits, what it does - and restate each option as a full sentence before its vote count; a label like "O3" or a shorthand like "the block" means nothing on its own. When in doubt, over-explain the context and cut the rest.
...
- Review panel: a fixed set of five models, each run at its highest available effort (`max`; Codex 5.5 has no `max` level, its ceiling is `xhigh` - use that) - Fable 5, Opus 5, Opus 4.8, Codex 5.5, Codex Sol. This is the panel the `docs/llm/FIXME.md` rounds refer to, and the one any "ask the panel" / "convene the reviewers" instruction means.
- Committee (user ruling): when the user says "convene the committee" / "ask the committee" (they may say it in Chinese), it means: open a FRESH, EMPTY session of EACH of these models and have them review or discuss the proposal at hand - Opus 4.8, Opus 5, Sonnet 4.6, Sonnet 5, plus Codex Sol and Codex 5.5 whenever the `codex` CLI is installed. Each member runs at its highest available effort (`max`; Codex 5.5 caps at `xhigh`), read-only, and gets the WHOLE brief in its prompt: an empty session knows nothing of this conversation, so the brief must stand on its own (the question, the contracts that constrain it, the candidate options with tradeoffs, and the repo paths to verify). Invocation from this devcontainer, brief piped on stdin, members run ONE AT A TIME (the RAM rule in the next bullet): `claude -p --model claude-opus-4-8 --effort max --allowedTools Read,Grep,Glob` (same for `claude-opus-5` / `claude-sonnet-4-6` / `claude-sonnet-5`; `--allowedTools` is variadic and swallows a positional prompt placed after it, hence stdin), `codex exec -m gpt-5.6-sol -c model_reasoning_effort=max -s read-only`, `codex exec -m gpt-5.5 -c model_reasoning_effort=xhigh -s read-only`. A member whose model id the installed CLI cannot resolve is skipped and the skip is reported to the user. The committee is the DISCUSSION body for design questions and proposals; the review panel above stays the FIX-REVIEW body for the `docs/llm/FIXME.md` rounds - the lists overlap on purpose and the escalation bullets below apply to both.
- When you hit a genuine fork - a design choice, a claim you cannot settle alone, anything where guessing wrong is costly - do NOT settle it solo: first convene the OTHER FOUR panel members (every panel model except the one you are running as) to review and discuss it together, by whatever channel reaches them. From this devcontainer all four are reachable directly and read-only: `codex exec -m gpt-5.6-sol -c model_reasoning_effort=max -s read-only` / `-m gpt-5.5 -c model_reasoning_effort=xhigh`, and `claude -p --model claude-opus-5 --effort max` / `--model claude-opus-4-8` (allow only `Read,Grep,Glob`); redirect stdin from `/dev/null` or the CLIs wait on it. Fall back to subagents or the user's relay only when a CLI is unavailable. Run the four CLIs ONE AT A TIME, and never alongside `npm run verify`: this devcontainer has about 1.9 GB of RAM, and two sessions died mid-turn while four reviewer CLIs ran concurrently next to a verify (the pattern of an OOM kill; the container exposes no dmesg / cgroup counters to confirm it).
