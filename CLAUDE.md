...
- One word, one concept - STRICT. Never reuse a word that already names one thing in this project for a second, confusable thing. Reserved: "label" appears in a name only where HTML itself calls the concept label (`<label>` -> `labelEl`, `aria-label` -> `ariaLabel`, `<optgroup label>` -> the groupLabel element family) - a display string is its TEXT, and the identity-to-string CONVERSION is `toString` (JS's own idiom). External vocabulary (ng-options' `label` clause, ui-select's `tagging-label`) may be quoted in backticks as the other project's term, mapped once to ours. Full ruling: `docs/llm/naming-conventions.md` s7a.7-8.
