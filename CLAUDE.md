...
- Commit messages: ONE line, in the house shape `type: [scope] terse summary`, clauses joined by `;` when needed - read `git log` for the register. No paragraph bodies, no rationale essays: the diff, docs, and FIXME/TODO entries carry the details. Trailers (e.g. Co-Authored-By) stay, after a blank line.
