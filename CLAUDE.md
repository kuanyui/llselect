...
- One word, one concept - STRICT. Never reuse a word that already names one thing in this project for a second, confusable thing. Reserved: "label" means the label-element / accessible-name family (`labelEl`, `ll-label-el`, `aria-label`) and NOTHING else; an item's or row's display string is its TEXT (`itemToString`, `ll-item-text`). External vocabulary (ng-options' `label` clause, ui-select's `tagging-label`) may be quoted in backticks as the other project's term, mapped once to ours.
