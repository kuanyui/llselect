...
- Docstrings and human-facing docs (TSDoc in src/, README, angularjs/API.md) are written for fast scanning - a reader with mild dyslexia must be able to follow them. Lead with ONE short sentence saying what the thing is, then put every further fact in its own bullet, one fact per bullet. Never glue several clauses into one sentence with dashes and nested parentheses. TSDoc renders verbatim into the generated API reference, so docstrings are user-facing documentation held to this rule, not internal comments. (Agent-facing docs under docs/llm/ keep their dense house style.)
