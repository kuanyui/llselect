...
Language packs are pure data under `@llselect/core/i18n` (50+ locales; `uiTranslationPackByLocale` indexes them by minimal BCP 47 tag): opt-in, tree-shakeable, zero behavior, so bundling translations does not violate "low-level". `en` is itself just the BUILT-IN pack; it lives in `src/ui-translation-pack.ts` (not i18n.ts) so both bundles can inline it cheaply - `i18n.ts` inlines only that tiny module, never `base.ts`, and the main bundle carries exactly one pack. Usage: `uiTranslationPack: zhTW`, or a per-key override on top: `uiTranslationPack: { ...zhTW, searchInputPlaceholder: '...' }`.
