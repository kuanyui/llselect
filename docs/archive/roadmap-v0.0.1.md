# Roadmap archive - v0.0.1

Status: **ARCHIVED - COMPLETED ROADMAP.** Every phase, ruling, and decision
below shipped in v0.0.1 and is recorded here for rationale-hunting only;
remaining work lives in `../TODO.md`. Normative contracts: `../DESIGN.md`
(API / architecture) and `../A11Y.md` (keyboard / focus / ARIA). Plain-text
file references inside the entries are decision-time paths relative to
`docs/` (e.g. `A11Y.md`); `optgroup-research.md` now sits in this directory.

Status markers kept as written: `[x]` done, `[~]` was in progress.
...
## API design decisions (all resolved)
...
- [x] **Release-readiness groundwork** (the completed slice; open release
      items live in `../TODO.md`): name availability checked (llselect is
      free), LICENSE, `prepublishOnly` build+test guard, version-drift test,
      `sideEffects` declaration (shipped as `false`, later corrected to
      `["**/*.css"]` so bundlers keep bare theme imports - REVIEW 1.3),
      `./package.json` export, README install / quick-start,
      `npm pack --dry-run` audited (dist + themes + d.ts + maps, README,
      LICENSE; ~149 kB tarball).
