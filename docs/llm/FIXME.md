...
- [x] **[QUALITY-81] - (rejected) forbid constructors from calling overridable methods; "solve" the constructor trap with two-phase init or a factory**
  - Symptom claimed: `LLSelectSingle` / `LLSelectMultiple` constructors call `this.renderTrigger()` right after `super()`, and the base constructor reaches `createTriggerClearButtonEl` / `createTriggerClearButtonContentEl` through the private `createTriggerEl` when `clearable`. A THIRD-level subclass's field initializers have not run yet, so an override reading its own fields sees `undefined` on that first render. MEDIUM-78 documented this; the reviewer wants it eliminated.
  - Fix: none - 5:0 REJECT of the prescription; MEDIUM-78 stands. The one real residue (the overridable method `rerender()` cannot repair) is QUALITY-89.
...
- [x] **[QUALITY-89] - the clear-button methods were the only construction-time overridable methods `rerender()` could not repair; hide-when-empty stays a theme rule**
  ...
  - Fix (owner ruling, part A): every trigger render rebuilds the clear button through `createTriggerClearButtonEl` (private `replaceTriggerClearButtonElInDom`: build, insert the new button, move focus to it when the old one held focus, then remove the old one), so the documented remedy - `rerender()` at the end of the extender's constructor - repairs this override like every other trigger method. `setUiTranslationPack` dropped its `querySelector` patch (the rebuild reads the new pack). The `renderTrigger` docstring's exception bullet and DESIGN.md's "one wrinkle" sentence are gone.
