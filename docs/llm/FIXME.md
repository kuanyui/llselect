...
- [x] **[MEDIUM-93] - a multiple's count summary kept the OLD list total after `setItems`**
  ...
  - Fix: both `onItemsChanged` re-render the trigger content (`renderTriggerContent`, not the arrow - it does not depend on the list) after every `setItems` (the multiple count summary reads the list total, and a custom `createTriggerContentElFn` receives `items` in both modes). A first cut re-rendered only while something was chosen; the third post-fix round (Codex 5.5) showed a custom trigger showing `0 / items.length` staying stale with nothing chosen, and single's custom content staying stale after a same-reference list refresh, so the rule is now unconditional - one content render per `setItems` is acceptable for a bulk call, and the docstrings disclose the tags cost (one chip per chosen item; while the popup is closed that IS the main work `setItems` does). The base `renderTrigger` docstring states exactly when it runs.
  - Verified: test/multiple.test.ts pins the refreshed default total and custom content with nothing chosen; test/compare-fn.test.ts pins single's custom content after a same-reference `setItems`.
