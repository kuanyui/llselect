...
- [x] **[QUALITY-89] - the clear-button methods were the only construction-time overridable methods `rerender()` could not repair; hide-when-empty stays a theme rule**
  ...
  - Owner's question answered: this was NOT about turning `clearable` off (a frozen capability by the DESIGN.md rule), and the "show it only when there is a value" wish is ALREADY the shipped behavior - in the themes.
  ...
  - Owner ruling, part B (rejected): hide-when-empty stays a theme rule on `data-empty` (consistent with the arrow's `:empty` rule; a custom theme author copies one line; near-zero downside). Core does not hide the button. The panel had split 4:1 for core-side hiding; the dissent (Opus 4.8) won on consistency.
  - Verified: test/clear.test.ts pins a subclass override that reads its own field (wrong at construction, repaired by `rerender()`), the button staying in its slot and functional after the rebuild, focus following the rebuilt button, and no button appearing without `clearable`, the popup staying open when `rerender()` runs with the button focused, a single value change and a multiple clear click (the button replaces itself inside its own handler) each yielding a fresh, working button; test/i18n.test.ts (assertions unchanged) pins the pack rename through the rebuild. Real-browser residue in TODO.md.
