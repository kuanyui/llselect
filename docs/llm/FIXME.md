...
- [x] **[MEDIUM-93] - a multiple's count summary kept the OLD list total after `setItems`**
  - Symptom: `setItems([A, B])` then `setChosenItems([A])` shows "1 / 2 selected"; a later `setItems([A, B, C])` still shows "1 / 2 selected" - the visible text and the accessible value both stale. Single is unaffected (its trigger shows the item's own text). Pre-existing; found by Opus 5 in the second post-fix round while checking the `renderTrigger` docstring claim "runs on every value / state change", which was false - `setItems` never re-rendered the trigger unless a chosen entry was dropped or swapped.
