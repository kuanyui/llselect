...
- [ ] **[QUALITY-89] - the clear-button methods are the only construction-time overridable methods `rerender()` cannot repair, and hide-when-empty exists only in the themes**
  - Symptom: src/base.ts `createTriggerEl` (:2241) builds the clear button once in the base constructor through the protected `createTriggerClearButtonEl` (:2276) / `createTriggerClearButtonContentEl` (:2302); `rerender()` (:1192) rebuilds trigger content and arrow only, so an extender override that reads its own fields keeps its first result forever - `setUiTranslationPack` (:1289) even patches the button's `aria-label` by `querySelector` because nothing else re-renders it. Separately, hide-when-empty is CSS in each shipped theme (`[data-empty='true'] .llselect-trigger-clear-button { display: none }`); core's `syncEmptyStateToDom` (:1500) writes only `data-empty`, so a custom theme shows a no-op "Clear selection" button while empty, although A11Y.md "Clear button" states it is hidden while nothing is selected.
  - Owner's question answered: this is NOT about turning `clearable` off (a frozen capability by the DESIGN.md rule), and the "show it only when there is a value" wish is ALREADY the shipped behavior - in the themes, not in core.
  - Proposed A (all five: the two methods must become `rerender()`-repairable; converged shape): keep a private `triggerClearButtonEl` ref, add a private `replaceTriggerClearButtonElInDom()` (build via `createTriggerClearButtonEl()`, `replaceWith`, re-focus the new button if the old one held focus), call it from `rerender()`; `setUiTranslationPack` drops its `querySelector` patch. Then the `renderTrigger` docstring "Exception" paragraph and the "one wrinkle" sentence in DESIGN.md "Customization model" delete, and the extender rule collapses to one line. NOT per `renderTrigger`: rebuilding a click-focusable button on every value change drops focus right after a clear click (that variant was proposed and withdrawn on this ground).
  - Proposed B (4:1): core hides the button while empty in `syncEmptyStateToDom` - the `hidden` attribute for semantics PLUS an inline `display` toggle, because every shipped theme sets `.llselect-trigger-clear-button { display: inline-flex }`, which beats the UA `[hidden]` rule (`hidden` alone would not hide it). `data-empty` stays as the CSS hook. Dissent (Opus 4.8): keep it in the themes like the arrow's `:empty { display: none }`, plus a custom-theme doc line. Awaits ruling on B.
  - Owner ruling: B rejected - hide-when-empty stays a theme rule on `data-empty` (consistent with the arrow's `:empty` rule; a custom theme author copies one line; near-zero downside). Core does not hide the button. A awaits the ruling; the owner asked for the plain-words restatement first.
