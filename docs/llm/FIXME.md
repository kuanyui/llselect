...
- [x] **[MEDIUM-93] - a multiple's count summary kept the OLD list total after `setItems`**
  ...
  - Fix: `LLSelectMultiple.onItemsChanged` re-renders the trigger after every `setItems` while something is chosen (the count summary reads the list total, and a custom `createTriggerContentElFn` receives `items`); nothing chosen = no render. The base `renderTrigger` docstring now states exactly when it runs.
  - Verified: test/multiple.test.ts pins the refreshed total and the no-render case with nothing chosen.
