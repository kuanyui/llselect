...
- [x] **[QUALITY-86] - (rejected) narrow the protected inheritance API into a renderer / adapter interface**
  - Fix: none - 5:0 REJECT. The `*Fn` settings already ARE the composed, content-only interface (deliberately unable to break ARIA); the protected methods are the extension layer, and their real users (the tree-select demo; the ui-select bridge wrapping `renderPopupList` / `renderTrigger` / `replacePopupListItemElInDom` / `close` for scope lifecycle) need `this` state, `super` chaining and lifecycle hooks a renderer object cannot express without growing into the same surface. An adapter object per instance also fights the mass-instantiation goal.
