...
- [x] **[QUALITY-88] - the AngularJS directives forwarded no open / close hook**
  ...
  - Fix: `ll-on-open` / `ll-on-close` on both directives (`wireEventAttr`): an expression evaluated on EACH event like `ng-click`, phase-safe (`$eval` inside a digest, `$apply` outside). API.md "Shared attributes" documents it as a fourth binding mode, **Event expression**; an expression error goes to `$exceptionHandler` (inside a digest `$eval` has no routing of its own), and the close that `destroy()` runs at scope teardown does not fire it (post-fix review, Opus 5).
  - Verified: angularjs tests pin the digest (an interpolated span updates on open and on close), a programmatic open from inside a digest (no nested `$apply`), an expression error inside a digest reaching `$exceptionHandler` with the open still completing (focus lands in the filter input), and no `ll-on-close` evaluation for the close that scope teardown runs.
