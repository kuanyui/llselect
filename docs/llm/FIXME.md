...
- [x] **[MEDIUM-93] - a multiple's count summary kept the OLD list total after `setItems`**
  ...
  - Fix: both `onItemsChanged` re-render the trigger content (`renderTriggerContent`; not the arrow, which does not depend on the list) after every `setItems`: the multiple count summary reads the list total, and a custom `createTriggerContentElFn` receives `items` in both modes. The base `renderTrigger` docstring states exactly when it runs.
  - Verified: test/multiple.test.ts pins the refreshed default total, custom content with nothing chosen, exactly one content render per `setItems` and no arrow render; test/compare-fn.test.ts pins single's custom content after a `setItems` that neither drops nor swaps the chosen item.
