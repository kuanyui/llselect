...
- [x] **[MEDIUM-76] - hosted inside an app's shadow root, option clicks read as outside clicks**
  ...
  - Verified: outside-click test hosts the widget in a shadow root and dispatches a composed pointer-down on an inside option (jsdom retargets `ev.target` to the host, confirmed by a probe); the popup stays open. Demonstrated failing on the pre-fix `ev.target`.
