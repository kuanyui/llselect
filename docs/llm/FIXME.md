...
- [x] **[QUALITY-89] - the clear-button methods were the only construction-time overridable methods `rerender()` could not repair; hide-when-empty stays a theme rule**
  ...
  - Fix (owner ruling, part A): `rerender()` rebuilds the clear button through `createTriggerClearButtonEl` (private `replaceTriggerClearButtonElInDom`: build, `replaceWith`, re-focus the new button when the old one held focus), so the documented remedy - `rerender()` at the end of the extender's constructor - repairs this override like every other trigger method. `setUiTranslationPack` dropped its `querySelector` patch (the rebuild reads the new pack). The `renderTrigger` docstring's exception bullet and DESIGN.md's "one wrinkle" sentence are gone.
  ...
  - Verified: test/clear.test.ts pins a subclass override that reads its own field (wrong at construction, repaired by `rerender()`), the button staying in its slot and functional after the rebuild, focus following the rebuilt button, and no button appearing without `clearable`; test/i18n.test.ts (unchanged) pins the pack rename through the rebuild.
  - Q: Why rebuild in `rerender()` only, not on every trigger render?
    - A: The arrow is re-created per render because its content varies with open state; the clear button does not vary with state and is click-focusable - replacing it on every value change would drop focus to `<body>` right after the clear click that caused the render. `rerender()` is the explicit, rare repaint, and it is the remedy the extender docs already prescribe.
