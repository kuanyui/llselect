...
Bulk-selection scope (user ruling): the `choose*` / `unchoose*` / `toggle*` bulk ops are UI-parity - they act on ENABLED items only and preserve already-chosen disabled items (bulk = many clicks, and clicking cannot reach a disabled item). The raw, disabled-blind total channel is `set*` / `clearSelection` (native `<select>` parity: programmatic assignment and clearing ignore disabled). So `all` in a bulk-op name means the whole ELIGIBLE (enabled) set - no `*AllEnabled` renames; the scope lives in this ruling plus each method's docstring.
...
- NAMED TYPE ALIASES are for enum-ish VALUE types only (`LLSelectOutsideClickBehavior`, `LLSelectTriggerDisplay`, `LLSelectChosenState`, `LLSelectWidthPolicy`, `LLSelectPlacement`) - callers declare variables of those. Callback types stay INLINE on the settings field: inside a settings literal, contextual typing infers them, so an alias buys nothing (the one historical callback alias was removed in the RC review).
- PREFIX (public exports): every exported TYPE and CLASS carries the `LLSelect` prefix - collision safety, since types surface in `.d.ts` / hover / error messages with no import context and generic names collide across libraries (`Placement` ships in both Popper and Floating UI). Exported FUNCTIONS (`createChevronDownSvgEl` ... `gatherItemsByGroupKey`, `createHighlightedTextEl`) and `version` do NOT: named imports namespace values, no reputable JS library prefixes standalone functions with its own name (date-fns `format`, MUI `createTheme`, Floating UI `computePosition`), and a consumer wanting the prefix writes `import * as LLSelect`. Five-model panel ruling, 5:0 (FIXME QUALITY-74).
...
### 4h. Popup header / footer slots

Settings on `LLSelectBaseSettings` (s3 by return type), Container-Content law (s7a.3): the library owns each container, the fn fills content. Both fns run ONCE, in the base constructor; the node is kept until `destroy()`.
...
| Vis       | Name                          | Convention                                                        |
| --------- | ----------------------------- | ----------------------------------------------------------------- |
| public    | `popupHeaderEl` / `popupFooterEl` | `*El` field, `HTMLElement \| null` - the container, or none built |
| protected | `createPopupHeaderEl` / `createPopupFooterEl` | `create*El` - whole container (override for full control) |
| protected | `createPopupHeaderContentEl` / `createPopupFooterContentEl` | `create*ContentEl` - thin, reads the setting |
| private   | `createPopupSlotEl`           | shared body of the two builders                                   |
...
| Vis       | Name                                   | Convention                                                          |
| --------- | -------------------------------------- | ------------------------------------------------------------------- |
| protected | `createPopupListLeadingActionRowEl` / `createPopupListTrailingActionRowEl` | `create*El` - whole row, one builder per position (s7d: explicit over one builder + kind param) |
...
3. **Container-Content law.** Every "library owns the container element + wiring; the hook fills only its visible content" customization point is named `create<Container>ContentElFn` (setting) + `create<Container>ContentEl` (protected, default reads the setting); `null` = that container's default content. Already conforming: trigger, item, tag, groupLabel. Brought into conformance by 7b: triggerArrow, triggerClearButton, tagRemoveButton. Later additions follow it: popupListNoResults (query-aware), chooseAllRow (tri-state + counts), popupHeader / popupFooter (called once at construction; the node is kept for the instance's life). Plain `create<Element>El` (no `Content`) builds the WHOLE element.
...
8. **Display-string vocabulary (user ruling): `String` is the canonical mechanism, `Text` is the UI-facing role.** `itemToString(Fn)` / `groupKeyToString(Fn)` say String because their output is the CANONICAL string of a datum - internal-leaning: it drives the default filter haystack and the accessible name as well as the default display, and it is never localizable (data-derived; switching `uiTranslationPack` must not change it). A name whose role at that point is "the text shown to the user" says Text: pack copy (`chooseAllRowText`), a pack message's interpolation parameter (`itemText` - what the pack author reads as "the item's displayed text"), the angularjs `ll-item-text` attribute (the template author stating what an item shows). The same value may flow through both vocabularies; the word follows the ROLE at that point, not the value's origin. ("to string" is also external JS vocabulary - `Object.prototype.toString`, `String(x)`; and `ll-item-to-string` was separately rejected because it would read as taking a function value like the `-fn` attributes do.)
9. **When two name forms are both grammatical, pick the one with only ONE reading** (user ruling). Idiom never outweighs ambiguity: a form that can be misread loses to a form that cannot, even when the unambiguous form is less common English. Instance (ruled here, ships with the open-state reader): the state reader is `isOpened()` with private field `opened`, NOT `isOpen` - "open" is both verb and adjective, and the verb sense is already taken by the `open()` action, so `isOpen` invites the wrong parse; "opened" reads one way only. The `createTriggerArrowContentElFn` ctx property renames with it (`{ isOpened: boolean }`). NOT cascaded: DOM/CSS state vocabulary (`data-state="open"`, `.llselect-open`) - attribute values and state classes have no verb namespace to collide with, and renaming them breaks themes.
...
| `chooseAllRow` (element, was `selectAllRow`) | user ruling: no exception to the `choose*` family - element, actions, pack key and CSS class all derive from it (`chooseAllRow`, `chooseAll`, `chooseAllRowText`, `.llselect-choose-all-row`). "select" appears only as the product noun (`LLSelect*`, `llselect-*`); the industry name "select all" survives as quoted prose and per-language pack VALUES (en still says "Select all (3 of 10)") |
