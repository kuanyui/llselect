...
Status: everything planned below has landed (commits `937e5db` to `9581257` on `dev`, then the fix round in `../FIXME.md`); this file is kept as the process record. The decisions and the reasons behind them are written for readers in `../popup-rows-and-callbacks.md`. Written for the next agent session (the previous session's context was about to compact). Everything below is either committed on the local `dev` branch (not pushed, not published: the npm 0.0.8 release predates all of it), or a decision the owner has ratified, or an open decision listed as such. The committee briefs and verbatim answers of rounds 5-9 lived only in the previous session's scratchpad; their outcomes are summarized here. Rounds 1-4 are in `popup-slots-research.md`.
...
- Action rows: `popupListLeadingActionRows` / `popupListTrailingActionRows`. App-defined commands, each rendered by the library as one `role="option"` row inside the listbox. Descriptor: `{ textFn, createContentElFn?, disabledFn?, onActivate }`.
- Leading rows: the choose-all row plus the leading action rows, together, at the top of the listbox. Trailing rows: the trailing action rows, at the bottom. The words name a position in the list, never time (the owner rejected before / after because they read as time).
- The ring: the arrow-key order: choose-all row, leading action rows, items, trailing action rows.
...
Demo rulings: 16.1 uses leading rows; no "Clear all" row (the choose-all row toggles); no "Load more" example (a paged list fights the filter); 16.3 is a sort toggle; 15.1 = tags in the trigger, count + a link-styled Clear all in the header; 15.4 = the 16.1 commands as header rows that LOOK like list rows (buttons wearing the theme's item class); a content fn must not read the instance variable (TDZ, `c65b1cc`).
...
- Round 9 (6:0 on everything below unless a split is noted):
  - Pin per BLOCK, never per row: two boolean settings, one for the leading rows, one for the trailing rows. The "row 1 and 3 pinned, row 2 not" case cannot be written. Per-row flags rejected (every fallback rule harms: reordering the ring, warn-and-degrade, per-row offset math).
  - Defaults `false` for both (the current look on `dev`). The choose-all row pins WITH the leading block (no separate flag). An empty pinned block builds no wrapper and no line.
...
- Open at the time of writing (all decided since: A-style names `popupListLeadingRowsPinned` / `popupListTrailingRowsPinned`, public wrapper fields with no builder to override, no event on the close reset, defaults `false`); vote counts from round 9:
  1. Flag names. 5 votes: `popupListLeadingRowsPinned: boolean` / `popupListTrailingRowsPinned: boolean`. 1 vote: verb form `pinPopupListLeadingRows` (like `hideChosenRows`).
  2. Wrapper element name and visibility. Proposals, in the leading / trailing vocabulary: `popupListPinnedLeadingRowsEl`, `popupListLeadingRowsPinnedEl`, `popupListLeadingRowsBlockEl` (+ the trailing twin); public 2, protected 2, private 2; a protected `create...El` override point: 3 for, 3 against. "leading" now names the block; the choose-all row and its hooks say choose-all.
  3. Whether `onFilterQueryChange` fires when `close()` resets a non-empty query (2 yes, 2 no, 2 silent).
  4. Keep the defaults at `false` (6:0 recommended; the owner may still flip the leading default later in its own turn per CLAUDE.md).
