...
- **Pinned blocks** (`popupListLeadingRowsPinned` / `popupListTrailingRowsPinned`, default `false`): per block, never per row (`popup-rows-and-callbacks.md` section 5). Mechanism: one `div role="presentation"` per pinned block (`popupListPinnedLeadingRowsEl` / `popupListPinnedTrailingRowsEl`), built once, in the listbox only while the block has rows; `position: sticky` and the edge offset are set inline so a boolean setting works in any theme; the themes own the opaque background, the `z-index` and the block's edge line (the top wrapper draws `border-bottom`, the bottom one `border-top`; the sibling divider rules keep serving unpinned blocks). `ensureVisibleInScroll` takes the wrappers' measured heights as insets, never a theme-declared number, and ignores them when they cover the whole scrollport. The choose-all row pins with the leading block; a subclass reaches the wrapper through the public field, there is no builder to override (pure structure, like `popupEl` and `popupListEl`).
