...
## Select-all default indicator (theme-drawn text glyph)

The select-all row's default tri-state indicator is a plain unicode TEXT glyph - ballot box `\2610`, squared minus `\229F`, checked ballot box `\2611` (with `\FE0E` forcing text presentation; `\2611` has an emoji variant in some font stacks) - drawn by each theme's CSS from `data-chosen-state`. It is NOT an icon-function SVG rendered by the core. Decided after briefly shipping `createOutlinedCheckboxSvgEl` as the core default and reverting:

- The core ships no icons. `icons.ts` is an opt-in leaf module; importing it from `multiple.ts` puts the checkbox paths into every multi bundle, `selectAllRow` on or off (the import sits in an always-present method, so tree shaking cannot drop it).
- The core must not pick a checkbox style. Outlined and filled are equal offerings (demo 5.5 / 5.6); making one the default would decide the app's look for it. That choice belongs to the caller (`createSelectAllRowContentElFn`) or to a batteries-included layer - the AngularJS package deliberately defaults to the outlined SVG via `ll-checkboxes`, and that is the right layer for such a default.
- Default looks live in themes, exactly like the clear / tag-remove `\00d7` cross. A theme can restyle or replace the glyph without touching JS.
- The glyph draws only while the row shows DEFAULT content: the row carries `data-content="default"` (custom content -> `"custom"`), set by the library. The first implementation gated with `:not(:has(*))` instead, and `:has()` sits above the browser support floor (Chrome 105+ / Safari 15.4+ / Firefox 121+ vs the 87 / 14.1 / 78 floor) - on floor browsers the whole rule was dropped and no indicator showed. The JS knows which branch it rendered, so it states it in an attribute.
