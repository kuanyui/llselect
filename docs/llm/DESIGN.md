...
The fix path is the Popover API (`popover="manual"` + `showPopover()`): the top layer paints above ALL stacking contexts and has no fixed-position containing block, and the element does NOT move in the DOM - every in-subtree invariant above keeps holding. It ships as a feature-detected enhancement over the current `position: fixed` fallback, with no browser-floor change (evergreen browsers get immunity, floor browsers keep exactly today's behavior). A body portal, by contrast, fixes only displacement - a body-appended popup still loses paint order to root-context competitors above its own `z-index`, renders UNDER an open `<dialog>` and its `::backdrop`, and forfeits the invariants above - so it stays rejected.
