...
- Match = `itemToString`, lower-cased `startsWith` - same text source and case handling as the filter's default compare (that compare is substring `includes`; this one is prefix). No diacritic folding, no `toLocaleLowerCase` (host-locale-dependent), and never `filterFn` (its contract is substring / user-defined; a fuzzy one would make everything match the first key).
