...
- The closed-state search starts relative to the COMMITTED selection (`typeaheadClosedStartIndex`: single overrides with the chosen item's index; default -1 = from the top), never relative to the convenience focus `focusInitial` parks - anchoring on that focus skips the first match (`['apple','avocado']` + "a" landed on avocado; caught by the post-fix panel round).
...
- A repeated-character buffer ("aa") always cycles like its single character - the native rule. The W3C APG example instead tries the literal doubled prefix first; rejected for native parity and simplicity (a text really starting "aa" is still reached by the cycle). The repeated-character test runs on the RAW characters, before lower-casing, so a key whose lower-case form expands (Turkish dotless-I family) still cycles.
