...
- **Choose-all stays its own setting** (round 4, 6:0): its tri-state, self-hiding, protected visible-enabled subset, pack text and AngularJS checkbox mode do not fit the generic descriptor; folding it in would grow the descriptor for one consumer. Revisit if creatable rows ever force a `visibleFn` in.
