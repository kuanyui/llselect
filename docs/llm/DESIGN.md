...
`popupListActionRowsBeforeItems` / `popupListActionRowsAfterItems` (both variants, default `[]`) render app commands as `role="option"` rows inside the listbox, in the arrow-key ring, before or after the items. The descriptor is `LLSelectPopupListActionRow { textFn; createContentElFn?; disabledFn?; onActivate }`; the library builds the element (id, role, classes, `aria-label`, `aria-disabled`, click, Enter) through `createPopupListActionRowBeforeItemsEl` / `createPopupListActionRowAfterItemsEl` (Container-Content law: `createPopupListActionRowContentEl` fills content, `isPopupListActionRowDisabled` reads the state, `onPopupListActionRowActivated` is the reaction hook). RULED after the committee rounds in `archive/popup-slots-research.md`:
...
- **One ring.** Navigation is one walk over [choose-all row, rows before items, items, rows after items] (`ringLength` / `ringPosition` / `ringEntryAt`), replacing the choose-all row's special-cased branches; the protected `focusedIndex` / `setFocusedIndex` / `focusLeadingRow` keep their item-based meaning. One visible change against the old special case: Page keys now clamp onto the choose-all row directly instead of stopping on the first item first.
